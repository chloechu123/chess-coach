...
def _worst_moments_slack(features: dict, limit: int = 4) -> str:
    ...
    for m in wm:
        ...
        motif = f", {m['motif']}" if m.get("motif") else ""
        lines.append(f"• Move {m.get('fullmove','?')} ({m.get('phase','')}{motif}): "
...
def _data_tables(features: dict) -> list:
    ...
    motifs = features.get("blunder_motifs") or {}
    if motifs:
        rows = [[k, str(v)] for k, v in motifs.items()]
        blocks += [_h("Blunder types"),
                   _para("What kind of mistake shows up most — the thing to actually drill."),
                   _table(["Motif", "Count"], rows)]
    ...
    if wm:
        ...
        blocks += [_h("Worst moments (flagged by Stockfish)"),
                   ...
                   _table(["Move", "Phase", "Motif", "Played", "Engine best", "Swing", "Game"], rows)]
